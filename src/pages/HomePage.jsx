...
export const HomePage = () => {
    ...
    const loadModelAndTokenizer = async () => {
        try {
            ...
            const loadedTokenizerConfig = await response.json();
            console.log("Tokenizer configuration loaded!");
            setTokenizerConfig(loadedTokenizerConfig);
    ...
    async function predictComment(processedComment) {
        if (model !== null) {
    ...
    const onSubmitAddPost = async ({ post = "" }) => {
        try {
            ...
            const predictChatGPT = await isCommentOffensive(post)
            console.log(predictedLabel)
            console.log(predictChatGPT)
...

            {
                isLoadingModel
                    ...
                    : <>
                        <Navbar />
                        <section
                            className="w-full md:w-10/12 lg:w-6/12 mx-auto mt-10"
                        >
                            <article
